      ...
      <div className="demo-header">
        <div className="demo-label">
          <span className="demo-sub">
            {examLabel} · {t('demo.question_label', { n: qIndex + 1, total: questions.length })}
          </span>
        </div>
...
function DemoResult({ answers, onStart, exam }) {
  ...
  const resultMsg = pct === 100
    ? t('demo.result_perfect')
    : pct >= 67
    ? t('demo.result_good')
    : t('demo.result_low');
  ...
  return (
    <div className="demo-result">
      ...
      <p className="demo-result-msg">{resultMsg}</p>
      <p className="demo-result-sub">
        {t('demo.result_score', { score, total })}
      </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, marginTop: 20 }}>
        <button className="btn btn-green btn-lg btn-full" onClick={() => onStart(exam)}>
          {t('demo.start_btn', { exam: exam === 'qudurat' ? t('common.qudurat') : t('common.tahsili') })}
        </button>
        <button
          onClick={() => onStart(alt)}
          style={{
        ...
        >
          {t('demo.alt_btn', { exam: alt === 'qudurat' ? t('common.qudurat') : t('common.tahsili') })} {t('common.arrow')}
        </button>
      </div>
  ...
  return (
    <div ref={ref} className="home-stats-row">
      <div className="home-stat">
        ...
        <span className="home-stat-label">{t('stats.questions_label')}</span>
      </div>
...
// ── Testimonials ──────────────────────────────────────────────────────────────
// Quotes stay in original language — they are real attributed quotes.
const TESTIMONIALS = [
  {
  ...
  {
    quote: "It's clear the questions have been carefully selected to deliver effective preparation for Qudurat and Tahsili.",
    name:  'Abdullah',
    role:  'School leader',
    exam:  'qudurat',
  },
  ...
  return (
    <div className="testi-strip" aria-label="Student testimonials">
      <div className="testi-track">
        {/* NOTE: variable renamed from 't' to 'testi' to avoid clash with useTranslation's t() */}
        {TESTI_LOOP.map((testi, i) => (
          <div key={i} className="testi-card">
            ...
            <div className="testi-footer">
              ...
              <span className={`testi-badge testi-badge-${testi.exam}`}>
                {testi.exam === 'qudurat' ? t('common.qudurat') : t('common.tahsili')}
              </span>
            </div>
          </div>
        ))}
...
// ── Value cards (kept as original constants — unused in render but preserved) ─
const VALUE_CARDS = [
  {
...
// ── Situation cards (kept as original — unused in render but preserved) ───────
...
// ── Problem bullets (kept as original — unused in render but preserved) ───────
...
// ── Situation SVGs (unchanged — decorative) ───────────────────────────────────
const SvgFast = () => (
...
const SvgStuck = () => (
  <svg width="120" height="120" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
    <style>{`
      @keyframes ssg-shake { 0%,18%,100%{transform:translate(0,0)} 20%{transform:translate(-2.5px,0)} 22%{transform:translate(2.5px,0)} 24%{transform:translate(-1.5px,0)} 26%{transform:translate(1.5px,0)} 28%{transform:translate(0,0)} }
      @keyframes ssg-needle { 0%,15%{transform:rotate(-78deg);transform-origin:60px 72px} 18%,19%{transform:rotate(-80deg);transform-origin:60px 72px} 20%,21%{transform:rotate(-76deg);transform-origin:60px 72px} 22%,75%{transform:rotate(-78deg);transform-origin:60px 72px} 80%{transform:rotate(-22deg);transform-origin:60px 72px} 85%,90%{transform:rotate(-20deg);transform-origin:60px 72px} 95%,100%{transform:rotate(-78deg);transform-origin:60px 72px} }
      @keyframes ssg-burst { 0%,74%,100%{opacity:0;transform:scale(0.5)} 80%,88%{opacity:1;transform:scale(1)} 93%{opacity:0;transform:scale(1.3)} }
      @keyframes ssg-ring { 0%,74%,100%{opacity:0;r:10} 79%{opacity:0.7;r:18} 88%{opacity:0.3;r:28} 94%{opacity:0;r:36} }
      @keyframes ssg-label { 0%,74%{fill:#64748B} 80%,92%{fill:#4ADE80} 97%,100%{fill:#64748B} }
      @keyframes ssg-score-in { 0%,10%{opacity:0;transform:translateY(5px)} 20%,70%{opacity:1;transform:translateY(0)} 80%,100%{opacity:0;transform:translateY(-5px)} }
    ...
    `}</style>
    <defs>
      <linearGradient id="ssg_bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#EFF6FF"/><stop offset="100%" stopColor="#E0F2FE"/></linearGradient>
      <linearGradient id="ssg_face" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#FFFFFF"/><stop offset="100%" stopColor="#F8FAFC"/></linearGradient>
      <linearGradient id="ssg_ndl" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#0F172A"/><stop offset="100%" stopColor="#475569"/></linearGradient>
      <radialGradient id="ssg_hub" cx="40%" cy="35%"><stop offset="0%" stopColor="#94A3B8"/><stop offset="100%" stopColor="#1E293B"/></radialGradient>
      <filter id="ssg_drop"><feDropShadow dx="0" dy="4" stdDeviation="6" floodColor="rgba(0,0,0,0.18)"/></filter>
    </defs>
    ...
    <g className="ssg-body">
      ...
      {[[17,80,23,77],[28,42,32,46],[60,22,60,28],[92,42,88,46],[103,80,97,77]].map(([x1,y1,x2,y2],i) => (
        <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#475569" strokeWidth="2.5" strokeLinecap="round"/>
      ))}
      {[[21,61,25,61],[22,71,26,71],[40,28,42,32],[50,23,51,27],[70,23,69,27],[80,28,78,32],[99,61,95,61],[98,71,94,71]].map(([x1,y1,x2,y2],i) => (
...
const SvgTrophy = () => (
  <svg width="120" height="120" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    `}</style>
    <defs>
      <linearGradient id="st3_bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#FFFBEB"/><stop offset="100%" stopColor="#FEF3C7"/></linearGradient>
      <linearGradient id="st3_cup" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#FDE68A"/><stop offset="45%" stopColor="#FBBF24"/><stop offset="100%" stopColor="#D97706"/></linearGradient>
      <linearGradient id="st3_cup_inner" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="rgba(0,0,0,0.04)"/><stop offset="100%" stopColor="rgba(0,0,0,0.12)"/></linearGradient>
      <linearGradient id="st3_stem" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#F59E0B"/><stop offset="100%" stopColor="#B45309"/></linearGradient>
      <linearGradient id="st3_base1" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#FBBF24"/><stop offset="100%" stopColor="#D97706"/></linearGradient>
      <linearGradient id="st3_base2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#D97706"/><stop offset="100%" stopColor="#92400E"/></linearGradient>
      <linearGradient id="st3_hdl" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#F59E0B"/><stop offset="100%" stopColor="#FCD34D"/></linearGradient>
      <linearGradient id="st3_shine_g" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="rgba(255,255,255,0)"/><stop offset="50%" stopColor="rgba(255,255,255,0.75)"/><stop offset="100%" stopColor="rgba(255,255,255,0)"/></linearGradient>
      <linearGradient id="st3_badge" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="rgba(255,255,255,0.32)"/><stop offset="100%" stopColor="rgba(255,255,255,0.12)"/></linearGradient>
      <radialGradient id="st3_hl" cx="35%" cy="30%"><stop offset="0%" stopColor="rgba(255,255,255,0.55)"/><stop offset="100%" stopColor="rgba(255,255,255,0)"/></radialGradient>
      <clipPath id="st3_cup_clip"><path d="M 36 20 Q 34 70 60 80 Q 86 70 84 20 Z"/></clipPath>
      <filter id="st3_dropshadow"><feDropShadow dx="0" dy="5" stdDeviation="5" floodColor="rgba(180,83,9,0.35)"/></filter>
    </defs>
    ...
    <g className="st3-star2"><path d="M102 14 L104 8 L106 14 L112 16.5 L106 19 L104 25 L102 19 L96 16.5 Z" fill="#FCD34D"/></g>
    <g className="st3-star3"><path d="M10 74 L12 68 L14 74 L20 76.5 L14 79 L12 85 L10 79 L4 76.5 Z" fill="#F59E0B" opacity="0.85"/></g>
...
// ── Situations — function taking t() so strings can be translated ─────────────
const getSituations = (t) => [
  {
    id:     'fast',
    label:  t('sit.fast.label'),
    title:  t('sit.fast.title'),
    body:   t('sit.fast.body'),
    cta:    t('sit.fast.cta'),
    detail: t('sit.fast.detail'),
    Svg:    SvgFast,
    color:  '#F97316',
  },
  {
    id:     'stuck',
    label:  t('sit.stuck.label'),
    title:  t('sit.stuck.title'),
    body:   t('sit.stuck.body'),
    cta:    t('sit.stuck.cta'),
    detail: t('sit.stuck.detail'),
    Svg:    SvgStuck,
    color:  '#3B82F6',
  },
  {
    id:     'top',
    label:  t('sit.top.label'),
    title:  t('sit.top.title'),
    body:   t('sit.top.body'),
    cta:    t('sit.top.cta'),
    detail: t('sit.top.detail'),
    Svg:    SvgTrophy,
    color:  '#F59E0B',
  },
];
...
function SituationSection({ onStart, selectedExam }) {
  ...
  const situations = getSituations(t);
    ...
    <section className="sit-section">
      <div className="home-container" style={{ position: 'relative', zIndex: 1 }}>
        <div className="sit-header">
          <div className="sit-eyebrow">{t('sit.eyebrow')}</div>
          <h2 className="sit-headline">
            {t('sit.headline_before')}
            <span className="sit-headline-accent">{t('sit.headline_accent')}</span>
            {t('sit.headline_after')}
          </h2>
          <p className="sit-subhead">{t('sit.subhead')}</p>
        </div>
        ...
        <div className="sit-card" style={{ borderColor: current.color + '40' }}>
          <div className="sit-card-left">
            ...
            >
              {current.cta} {t('common.arrow')}
            </button>
          </div>
          <div className="sit-card-right">
            <div className="sit-card-svg"><current.Svg /></div>
...
export default function Home() {
  ...
  const altExamName   = altExam === 'tahsili' ? t('common.tahsili') : t('common.qudurat');
        ...
        <div className="hero-inner">
          <div className={`hero-content ${heroVisible ? 'anim-in' : ''}`}>
            ...
            <h1 className="hero-title">
              {t('hero.title_1')}<br />
              {t('hero.title_2')} <span className="hero-title-accent">{t('hero.title_accent')}</span><br />
              {t('hero.title_3')}
            </h1>
            ...
            {/*
              ...
              wider) instead of flowing inline next to each other.
            */}
            <div style={{
              display:        'flex',
      ...
      <section className="prob-section">
        ...
        <div className="home-container" style={{ position: 'relative', zIndex: 1 }}>
          <div className="prob-header">
            <div className="prob-eyebrow">{t('prob.eyebrow')}</div>
            <h2 className="prob-headline">
              {t('prob.headline_before')}
              <span className="prob-headline-accent">{t('prob.headline_accent')}</span>
              {t('prob.headline_after').split('\n').map((line, i) => (
                <React.Fragment key={i}>{i > 0 && <br />}{line}</React.Fragment>
              ))}
            </h2>
            <p className="prob-subhead">{t('prob.subhead')}</p>
          </div>

          <div className="prob-cards">
            {[
              { num: '01', Illustration: IllustrationNoPath,       titleKey: 'prob.card1_title', textKey: 'prob.card1_text' },
              { num: '02', Illustration: IllustrationNoVisibility, titleKey: 'prob.card2_title', textKey: 'prob.card2_text' },
              { num: '03', Illustration: IllustrationWastedEffort, titleKey: 'prob.card3_title', textKey: 'prob.card3_text' },
            ].map((item, i) => (
              <div key={item.num} className="prob-card" style={{ animationDelay: `${i * 120}ms` }}>
                <div className="prob-card-svg-wrap"><item.Illustration /></div>
                <h3 className="prob-card-title">{t(item.titleKey)}</h3>
                <p className="prob-card-text">{t(item.textKey)}</p>
              </div>
            ))}
          </div>
          ...
          <div className="prob-divider">
            ...
            <div className="prob-divider-badge">{t('prob.divider')}</div>
            <div className="prob-divider-line" />
          </div>
          ...
          <div className="prob-solution">
            ...
            <div className="prob-solution-inner">
              <div className="prob-solution-icon">{t('common.arrow')}</div>
              <div>
                <p className="prob-solution-main">{t('prob.solution_main')}</p>
                <p className="prob-solution-sub">{t('prob.solution_sub')}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="val-section">
        <div className="home-container" style={{ position: 'relative', zIndex: 1 }}>
          <div className="val-header">
            <div className="val-eyebrow">{t('val.eyebrow')}</div>
            <h2 className="val-headline">
              {t('val.headline_before')}
              <span className="val-headline-accent">{t('val.headline_accent')}</span>
              {t('val.headline_after')}
            </h2>
            <p className="val-subhead">{t('val.subhead')}</p>
          </div>
          ...
          <div className="val-cards">
            ...
            <div className="val-card">
              <div className="val-card-svg-wrap">
                <svg width="150" height="150" viewBox="0 0 150 150" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <style>{`
                    @keyframes dot-travel { 0%{offset-distance:0%;opacity:1} 90%{offset-distance:100%;opacity:1} 95%{opacity:0} 100%{offset-distance:100%;opacity:0} }
                    @keyframes path-draw { from{stroke-dashoffset:400} to{stroke-dashoffset:0} }
                    @keyframes node-pop { 0%,80%{transform:scale(0.7);opacity:0.3} 85%{transform:scale(1.2);opacity:1} 100%{transform:scale(1);opacity:1} }
                    @keyframes checkmark-in { 0%,75%{opacity:0;transform:scale(0)} 85%{opacity:1;transform:scale(1.2)} 100%{opacity:1;transform:scale(1)} }
                    @keyframes glow-pulse { 0%,100%{opacity:0.4;r:8px} 50%{opacity:0.9;r:12px} }
                    .val-road{animation:path-draw 2s ease forwards;stroke-dasharray:400}
                    .val-dot{offset-path:path('M 30 120 Q 50 90 75 75 Q 100 60 110 40');animation:dot-travel 3s ease-in-out infinite}
                  ...
                  <g className="val-node1"><circle cx="30" cy="120" r="10" fill="#15803D" stroke="white" strokeWidth="2.5"/><text x="30" y="124" textAnchor="middle" fill="white" fontSize="10" fontWeight="800" fontFamily="Arial">1</text></g>
                  <g className="val-node2"><circle cx="75" cy="75" r="10" fill="#15803D" stroke="white" strokeWidth="2.5"/><text x="75" y="79" textAnchor="middle" fill="white" fontSize="10" fontWeight="800" fontFamily="Arial">2</text></g>
                  <g className="val-node3"><circle cx="110" cy="40" r="12" fill="#15803D" stroke="#4ADE80" strokeWidth="3"/><text x="110" y="44" textAnchor="middle" fill="white" fontSize="10" fontWeight="800" fontFamily="Arial" className="val-check">✓</text></g>
              ...
              <h3 className="val-card-title">{t('val.card1_title')}</h3>
              <p className="val-card-text">{t('val.card1_text')}</p>
            </div>
            ...
            <div className="val-card val-card-featured">
              <div className="val-card-svg-wrap">
                <svg width="150" height="150" viewBox="0 0 150 150" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <style>{`
                    @keyframes glass-scan { 0%{transform:translate(0px,0px)} 20%{transform:translate(10px,8px)} 40%{transform:translate(4px,22px)} 60%{transform:translate(14px,36px)} 75%{transform:translate(6px,36px)} 90%{transform:translate(6px,36px)} 100%{transform:translate(0px,0px)} }
                    @keyframes topic-weak1 { 0%,30%{fill:#E2E8F0} 35%,85%{fill:#FEE2E2} 90%,100%{fill:#E2E8F0} }
                    @keyframes topic-weak2 { 0%,55%{fill:#E2E8F0} 62%,85%{fill:#FEE2E2} 90%,100%{fill:#E2E8F0} }
                    @keyframes topic-found { 0%,58%{fill:#E2E8F0;stroke:transparent;stroke-width:0} 65%,85%{fill:#FEE2E2;stroke:#EF4444;stroke-width:2} 90%,100%{fill:#DCFCE7;stroke:#15803D;stroke-width:2} }
                    ...
                    @keyframes exclaim-pulse { 0%,30%{opacity:0} 35%,50%{opacity:1;transform:scale(1.2);transform-origin:108px 56px} 55%,84%{opacity:1;transform:scale(1);transform-origin:108px 56px} 90%,100%{opacity:0} }
                    @keyframes exclaim2-pulse { 0%,55%{opacity:0} 62%,75%{opacity:1;transform:scale(1.2);transform-origin:108px 86px} 80%,84%{opacity:1;transform:scale(1);transform-origin:108px 86px} 90%,100%{opacity:0} }
                    @keyframes glow-ring { 0%,58%{opacity:0;r:28} 65%,75%{opacity:0.6;r:32} 82%,84%{opacity:0.3;r:36} 90%,100%{opacity:0;r:28} }
                    @keyframes score-jump { 0%,85%{opacity:0;transform:translateY(0)} 90%{opacity:1;transform:translateY(-6px)} 100%{opacity:0;transform:translateY(-14px)} }
                    .mg-glass{animation:glass-scan 4s cubic-bezier(0.4,0,0.2,1) infinite}
                    .t-row1{animation:topic-weak1 4s ease infinite}.t-row2{animation:topic-weak2 4s ease infinite}.t-row3{animation:topic-found 4s ease infinite}
                    .l-row1{animation:label-weak1 4s ease infinite}.l-row2{animation:label-weak2 4s ease infinite}.l-row3{animation:label-found 4s ease infinite}
                    .chk{animation:check-appear 4s ease infinite}.exc1{animation:exclaim-pulse 4s ease infinite}.exc2{animation:exclaim2-pulse 4s ease infinite}
                    .glow-r{animation:glow-ring 4s ease infinite}.score-pop{animation:score-jump 4s ease infinite}
                  `}</style>
                  <defs>
                  ...
                  <g className="score-pop"><rect x="52" y="90" width="46" height="16" rx="6" fill="#15803D"/><text x="75" y="101" textAnchor="middle" fill="white" fontSize="9" fontWeight="800" fontFamily="Arial">+18 pts!</text></g>
              ...
              <h3 className="val-card-title">{t('val.card2_title')}</h3>
              <p className="val-card-text">{t('val.card2_text')}</p>
              <div className="val-card-badge">{t('val.card2_badge')}</div>
            </div>
              ...
              <div className="val-card-svg-wrap">
                <svg width="150" height="150" viewBox="0 0 150 150" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <style>{`
                    ...
                    .vb1{animation:bar1-grow 3s ease-in-out infinite}.vb2{animation:bar2-grow 3s ease-in-out infinite}
                    .vb3{animation:bar3-grow 3s ease-in-out infinite}.vb4{animation:bar4-grow 3s ease-in-out infinite}
                  ...
                  <g className="vstar"><rect x="92" y="18" width="36" height="18" rx="6" fill="#FBBF24"/><text x="110" y="30" textAnchor="middle" fill="white" fontSize="9" fontWeight="800" fontFamily="Arial" className="vscore">+42 pts</text></g>
                </svg>
              </div>
              <h3 className="val-card-title">{t('val.card3_title')}</h3>
              <p className="val-card-text">{t('val.card3_text')}</p>
            </div>
          </div>
          ...
          <div className="val-cta">
            ...
            <p className="val-cta-sub">{t('val.cta_sub')}</p>
          </div>
        </div>
      </section>

      {/* ── 6. BUILT FOR YOUR SITUATION ──────────────────────────────────── */}
      ...
      <section className="cmp-section">
        <div className="home-container" style={{ position: 'relative', zIndex: 1 }}>
          <div className="cmp-header">
            <div className="cmp-eyebrow">{t('cmp.eyebrow')}</div>
            <h2 className="cmp-headline">
              {t('cmp.headline_1')}<br />{t('cmp.headline_2')}
            </h2>
            <p className="cmp-subhead">{t('cmp.subhead')}</p>
          </div>
          ...
          <div className="cmp-grid">
            <div className="cmp-col cmp-col-bad">
              <div className="cmp-col-header">
                ...
                <h3 className="cmp-col-title cmp-col-title-bad">{t('cmp.bad_title')}</h3>
              </div>
              <div className="cmp-items">
                {Array.isArray(cmpBad) && cmpBad.map((item, i) => (
                  <div key={i} className="cmp-item cmp-item-bad" style={{ animationDelay: `${i * 100}ms` }}>
            ...
            <div className="cmp-col cmp-col-good">
              <div className="cmp-col-header">
                ...
                <h3 className="cmp-col-title cmp-col-title-good">{t('cmp.good_title')}</h3>
              </div>
              <div className="cmp-items">
                {Array.isArray(cmpGood) && cmpGood.map((item, i) => (
                  <div key={i} className="cmp-item cmp-item-good" style={{ animationDelay: `${i * 100}ms` }}>
          ...
          <div className="cmp-footer">
            <p className="cmp-closing">
              {t('cmp.closing_1')} <strong>{t('cmp.closing_2')}</strong>
            </p>
            <button className="btn btn-green btn-lg" onClick={() => handleStart(selectedExam)}>
              {t('cmp.cta')}
            </button>
        ...
        <div className="home-container" style={{ position: 'relative', zIndex: 1 }}>
          <div className="fcta-inner">
            ...
            <h2 className="fcta-headline">
              {t('fcta.headline_1')}<br />
              <span className="fcta-headline-accent">{t('fcta.headline_accent')}</span>
            </h2>
            <p className="fcta-sub">{t('fcta.sub')}</p>
            <div className="fcta-trust-row">
              {['fcta.trust_1','fcta.trust_2','fcta.trust_3','fcta.trust_4'].map((key) => (
                <span key={key} className="fcta-trust-pill">{t(key)}</span>
              ))}
            </div>
            <div className="fcta-buttons">
              <button className="fcta-btn-primary" onClick={() => handleStart(selectedExam)}>
                {t('fcta.btn_primary', { exam: examName })}
              </button>
              <button className="fcta-btn-secondary" onClick={() => handleStart(altExam)}>
                {t('fcta.btn_secondary', { exam: altExamName })}
              </button>
            </div>
            <p className="fcta-login">
              {t('fcta.login_text')}{' '}
              <Link to="/login" className="fcta-login-link">{t('fcta.login_link')}</Link>
            </p>
          </div>
        </div>
      </section>
