...
resolve_world_access(user, exam, world_key)
  → {"allowed": bool, "lock_reason": str | None}

resolve_level_access(user, exam, world_key, level_number)
  → {"allowed": bool, "lock_reason": str | None}
...
TRACK-AWARE:
  Prerequisite checks are within-track only.
  Entitlement max_world_index is compared against track-relative index.
  Trial world limit is per-track (first 2 worlds in EACH track).
"""
...
def _get_active_org_entitlement(user, exam: str, track_world_index: int):
    """
    Returns an active org-level Entitlement covering this exam + track_world_index,
    ...
    return (
        ...
        .filter(
            ...
            Entitlement.max_world_index >= track_world_index,
...
def _get_active_individual_entitlement(user, exam: str, track_world_index: int):
    """
    Returns an active individual Entitlement for this user covering
    exam + track_world_index, or None.
    """
    now = datetime.now(timezone.utc)
    return (
        Entitlement.query
...
# ── Progression checks (TRACK-AWARE) ─────────────────────────────────────────

def _prev_world_completed(user_id: int, exam: str, world_key: str) -> bool:
    """
    Returns True if the previous world WITHIN THE SAME TRACK is completed,
    or True if this is the first world in the track.

    Track-aware: math_200 checks math_150, NOT verbal_100.
    """
    prev_world_key = get_prev_world_in_track(exam, world_key)
    if prev_world_key is None:
        return True  # first world in track — no prerequisite
...
def resolve_world_access(user, exam: str, world_key: str) -> dict:
    """
    Determine whether user can access a world.

    Uses track-relative world index for entitlement/trial checks.
    Uses within-track progression for prerequisite checks.
    ...
    # 1. Org entitlement
    org_ent = _get_active_org_entitlement(user, exam, track_world_index)
    if org_ent:
        if not _prev_world_completed(user.id, exam, world_key):
            return {"allowed": False, "lock_reason": LockReason.PREREQ_INCOMPLETE}
        return {"allowed": True, "lock_reason": None}
    ...
    ind_ent = _get_active_individual_entitlement(user, exam, track_world_index)
    if ind_ent:
        if not _prev_world_completed(user.id, exam, world_key):
            return {"allowed": False, "lock_reason": LockReason.PREREQ_INCOMPLETE}
        return {"allowed": True, "lock_reason": None}
